...
# ── Redis-backed Lobby Helpers ────────────────────────────────────────────────
def save_lobby(lob: Lobby):
    """Persist lobby JSON + expire"""
...
def new_game(req: NewReq = Body(...)):
    ...
    SOLO_GAMES[gid] = {'mode': req.mode, 'board': board, 'status': 'in_progress', 'erase': engine}
...
def queue(
    ...
    engine = erase_mod.EraseEngine(board) if req.mode == 'erase' else None
    lob = Lobby(id=str(uuid4()), mode=req.mode, players=[pid], board=board, erase_engine=engine)
    save_lobby(lob)
...
def guess(req: GuessReq = Body(...)):
    ...
    score = lambda p: (abs(guesses[p]-target), -guesses[p])
...
def state(
    ...
    opponent = lob.players[1] if side=='x' else lob.players[0]
    return PvPState(lobby_id=lobby_id, board=lob.board, status=lob.status,
                    your_side=side, turn=lob.turn, opponent=opponent,
                    score_x=lob.score_x, score_o=lob.score_o)
...
def pvp_move(
    ...
    if lob.mode == 'experimental':
        exp_mod.check_instant_win(lob.board)
        # accumulate 3-in-a-row points if needed
    else:
        lob.status = classic.status(lob.board)
    if lob.status=='x_wins': lob.score_x += 1
    if lob.status=='o_wins': lob.score_o += 1
    if lob.status=='in_progress': lob.turn = 1-idx
    save_lobby(lob)
